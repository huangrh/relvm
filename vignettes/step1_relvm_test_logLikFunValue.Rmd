...
venll15(par=init,score=mstbl_std, wts=wts_tbl,qpoints) - 22053.166744270311 #use final parameters. 
# microbenchmark::microbenchmark(re1 = venll15(par=init,score=mstbl_std, wts=wts_tbl,cc,qpoints))
# l <- lineprof::lineprof(venll15(par=init,score=mstbl_std, wts=wts_tbl,cc,qpoints))
...
v=1
# speedup Vectorized Estimation function
venll14 <- function(par,score,wts,cc,qpoints) {
    # Reconstruction of the parameters
    nr <- nrow(score); nc <- ncol(score)
    mu    <- par[grepl("mu", names(par))]         #
    fl    <- par[grepl("fl", names(par))]         # factor loading
    ...
    # 3D array: 
    wts_arr   <- array(wts,  dim=c(nr,nc,qpoints))
    score_arr <- array(score,dim=c(nr,nc,qpoints))
    means_arr <- array(rep(mu,rep(nr,nc)), dim=c(nr,nc, qpoints)) + coefs %o% fl %o% cc$x
    
    # Weighted log likelyhood
    wll_arr <- array(c(wts) * dnorm2(c(score), mean=c(means_arr), sd = rep(err,rep(nr,nc))),c(nr,nc,qpoints))
    wll_mtx <- colSums(aperm(wll_arr,c(2,3,1)),na.rm=TRUE)
...
venll14(par=init,score=mstbl_std, wts=wts_tbl,cc,qpoints)
microbenchmark::microbenchmark(re1 = venll14(par=init,score=mstbl_std, wts=wts_tbl,cc,qpoints))
# l <- lineprof::lineprof(venll14(par=init,score=mstbl_std, wts=wts_tbl,cc,qpoints))
...
venll13(par=init,score=mstbl_std, wts=wts_tbl,cc,qpoints)
...
v=1
# speedup Vectorized Estimation function
venll12 <- function(par,score,wts,fv,logccw,qpoints) {
    # Reconstruction of the parameters
    nr <- nrow(score); nc <- ncol(score)
    mu    <- par[grepl("mu", names(par))]         #
    fl    <- par[grepl("fl", names(par))]         # factor loading
    ...
    # log(2*pi)/2 = 0.91893853320467267=dnorm_cpp(cc$x * sqrt(2), mean=0,sd=1) +(cc$x)^2; 
    # log(sqrt(2))=0.3465735902799727
    gqi <- matrixStats::colLogSumExps(logccw + wll_mtx - 0.91893853320467267,na.rm=TRUE)
    -sum(gqi +0.3465735902799727, na.rm=TRUE)
}
venll12(par=init,score=mstbl_std, wts=wts_tbl,fv,logccw,qpoints)
microbenchmark::microbenchmark(re1 = venll12(par=init,score=mstbl_std, wts=wts_tbl,fv,logccw,qpoints))
# l <- lineprof::lineprof(venll12(par=init,score=mstbl_std, wts=wts_tbl,fv,logccw,qpoints))
...
venll11 <- function(par,score,wts,cc,qpoints) {
    ...
    # 3D array: 
    wts_arr   <- aperm(array(t(wts),  dim=c(nc,nr,qpoints)),c(1,3,2))
    score_arr <- aperm(array(score,dim=c(nr,nc,qpoints)),c(2,3,1))
    means_arr <- array(mu,         dim=c(nc,qpoints,nr)) + fl %o% fv_mtx
