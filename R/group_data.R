...
subgroup <- function(object,groups=NULL) {
  ...
  alldata<- sapply(groups, sub1group, df=alldf, simplify = FALSE)
  alldata
}


#
sub1group <- function(group,df=alldf) {
  ...
  dat_tbl     <- df[, names(df) %in% measure_name]

  # weight table
  wt_name <- sapply(measure_name, function(x) paste0(x,"_wt"))
  wt_tbl  <- df[, names(df) %in% wt_name]
